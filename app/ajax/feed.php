      ...
      if (in_array($a, Posts::$availableActions)) {
         if ($a == "sendForm") {
            ...
            if (isset($_GET['mode'])) {
               ...
               $posts = DB::query('SELECT posts.id, posts.userId, posts.content, posts.likes, posts.shares, posts.comments, posts.createdAt, posts.privacy, users.profileImg, users.users_id AS users_id FROM posts, users WHERE users_id = userId AND privacy = 1 OR (userId = users_id AND privacy LIKE "%") ORDER BY posts.createdAt DESC LIMIT 5 OFFSET '.$start, [':userid' => $user['id']]);
            }
